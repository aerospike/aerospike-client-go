...
func concretePackObjectReflect(cmd BufferEx, obj interface{}, mapKey bool) (int, error) {
	...
	case reflect.Array, reflect.Slice:
		if mapKey && reflect.TypeOf(obj).Kind() == reflect.Slice {
			return 0, NewAerospikeError(types.SERIALIZE_ERROR, fmt.Sprintf("Maps, Slices, and bounded arrays other than Bounded Byte Arrays are not supported as Map keys. Value: %#v", obj))
		}
	...
	return 0, NewAerospikeError(types.SERIALIZE_ERROR, fmt.Sprintf("Type `%#v` not supported to pack.", obj))
}
