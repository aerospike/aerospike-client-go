...
const (
	...
	expTraceKeyOutcome    = 7  // eval-phase outcome (uint; reserved, SERVER-1138)
	expTraceKeyLang       = 8  // source language (uint; ExpTraceLangMsgpack / ExpTraceLangAel)
	expTraceKeyAelOffset  = 9  // char offset into AEL source text (uint; reserved)
	expTraceKeyAelSpan    = 10 // byte width of the offending AEL source region (uint; reserved)
...
const (
	...
	// ExpTracePhaseEval indicates expression evaluation failed (reserved, SERVER-1138).
	ExpTracePhaseEval = 2

	// ExpTraceLangMsgpack is the msgpack source language (the implied default when
	// the lang key is absent).
	ExpTraceLangMsgpack = 1
	// ExpTraceLangAel is the AEL DSL source language (reserved for a future server branch).
	ExpTraceLangAel = 2
...
// [BasePolicy.ErrorDetailVerbosity]) and the server fails to build an expression -
// a metadata/predicate filter (filter_exp) or an exp_read/exp_write operation - it
...
// Snippet first, then Path, when the budget is tight, so those may be absent even
// within a present trace. Absent integer fields read as -1 (except Lang, which
// defaults to [ExpTraceLangMsgpack]); absent string fields read as "" and an absent
// Path reads as nil. Never require any field.
//
...
// into AEL source text - a different coordinate space, reserved for a future server
// branch and absent on today's msgpack build traces.
type ExpressionTrace struct {
	// Phase that failed: [ExpTracePhaseBuild] or [ExpTracePhaseEval]; -1 when absent.
	// Today the server emits build traces only (ExpTracePhaseBuild).
	Phase int
	...
	// absent (it is the first field the server drops under a tight byte budget).
	Snippet string
	...
	// Reserved for the AEL DSL branch; absent on today's msgpack build traces. A
	// different coordinate space from ByteOffset.
	AelOffset int
