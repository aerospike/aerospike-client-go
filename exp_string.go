...
// addStringModifyBytes returns a string-modify expression whose argument list
// has already been pre-packed (used by replace / replaceAll / regexReplace,
// which need a 2-element nested list inside their argument array).
func addStringModifyBytes(src *Expression, bytes []byte) *Expression {
	flags := int64(_stringExpMODULE | _MODIFY)
	return &Expression{
		cmd:       &expOpCALL,
		val:       nil,
		...
		module:    &ExpTypeSTRING,
		exps:      nil,
		arguments: []ExpressionArgument{stringExpRawArgs(bytes)},
	}
}

// stringExpRawArgs is an ExpressionArgument that emits a pre-packed argument
// list verbatim. The expression encoder writes the supplied bytes in place of
// the usual `packArrayBegin(argLen)` + per-arg pack pair, so the caller is
// responsible for including the outer array header.
type stringExpRawArgs []byte

func (r stringExpRawArgs) pack(buf BufferEx) (int, Error) {
	if buf == nil {
		return len(r), nil
	}
	return buf.Write([]byte(r))
}

// packStringReplaceExp builds the pre-packed argument bytes for replace /
// replaceAll expressions:
//
//	[cmd, [needle, repl], flags]
//
// needle/replacement are nested inside a 2-element list. Two passes (size
// estimation then write) match how Java's StringExp.packReplace operates and
// how the other expression argument builders in this client estimate then write.
func packStringReplaceExp(command int, needle *Expression, replacement *Expression, flags int) ([]byte, Error) {
	sz, err := writeReplaceExp(nil, command, needle, replacement, flags)
	if err != nil {
		return nil, err
	}
	buf := newBuffer(sz)
	if _, err = writeReplaceExp(buf, command, needle, replacement, flags); err != nil {
		return nil, err
	}
	return buf.Bytes(), nil
}

func writeReplaceExp(buf BufferEx, command int, needle *Expression, replacement *Expression, flags int) (int, Error) {
	size := 0
	n, err := packArrayBegin(buf, 3)
	if err != nil {
		return size + n, err
	}
	size += n

	n, err = packAInt(buf, command)
	if err != nil {
		return size + n, err
	}
	size += n

	n, err = packArrayBegin(buf, 2)
	if err != nil {
		return size + n, err
	}
	size += n

	n, err = needle.pack(buf)
	if err != nil {
		return size + n, err
	}
	size += n

	n, err = replacement.pack(buf)
	if err != nil {
		return size + n, err
	}
	size += n

	n, err = packAInt(buf, flags)
	if err != nil {
		return size + n, err
	}
	size += n
...
// packStringRegexReplaceExp builds the pre-packed argument bytes for
// regexReplace expressions:
//
//	[REGEX_REPLACE, [pattern, repl], regexFlags]
//
// The server's regex_replace op table accepts only [list, regexFlags] (no slot
// for policy flags), so this layout matches Java's StringExp.packRegexReplace.
func packStringRegexReplaceExp(pattern *Expression, replacement *Expression, regexFlags int) ([]byte, Error) {
	sz, err := writeReplaceExp(nil, _STR_OP_REGEX_REPLACE, pattern, replacement, regexFlags)
	if err != nil {
		return nil, err
	}
	buf := newBuffer(sz)
	if _, err = writeReplaceExp(buf, _STR_OP_REGEX_REPLACE, pattern, replacement, regexFlags); err != nil {
		return nil, err
	}
	return buf.Bytes(), nil
}
