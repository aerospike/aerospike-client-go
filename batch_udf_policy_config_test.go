...
var _ = Describe("ApplyConfigToBatchUDFPolicy", func() {

	Context("when applying full configuration to batch udf policy", func() {
	...
	Context("when applying batch read config to a write policy", func() {
		It("should update the write policy values based on the batch udf dynamic config", func() {
			// Create the full configuration.
			config := &DynConfig{
				config: &dynconfig.Config{
					Dynamic: &dynconfig.DynamicConfig{
